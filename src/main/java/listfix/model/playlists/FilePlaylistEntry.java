...
public class FilePlaylistEntry extends PlaylistEntry {
  ...
  public void setFileName(String filename) {
    // Use the raw filename for file system operations; normalization is applied
    // upon retrieval
  ...
  private boolean skipExistsCheck() {
    ...
    return isFound()
        || ArrayFunctions.containsStringPrefixingAnotherString(
            emptyPaths, this.getTrackFolder(), false);
  }

  public boolean findNewLocationFromFileList(
      Collection<String> fileList, boolean caseInsensitiveExactMatching, boolean useRelativePath) {
    ...
    for (String file : fileList) {
      // Get the filename from the media library entry for direct comparison
      // (normalized)
      candidateFileName = UnicodeUtils.normalizeNfc(Path.of(file).getFileName().toString());
      if (caseSensitiveMatching
          ? candidateFileName.equals(trimmedFileName)
          : candidateFileName.equalsIgnoreCase(trimmedFileName)) {
  ...
  public boolean updatePathToMediaLibraryIfFoundOutside(
      IMediaLibrary dirLists, boolean caseInsensitiveExactMatching, boolean useRelativePath) {
    if (_status == PlaylistEntryStatus.Found
        && !ArrayFunctions.containsStringPrefixingAnotherString(
            dirLists.getMediaDirectories(), this.getTrackFolder(), FilePlaylistEntry.isWindows)) {
      return findNewLocationFromFileList(
          dirLists.getNestedMediaFiles(), caseInsensitiveExactMatching, useRelativePath);
    }
    return false;
  }
  ...
  public void update(Path track) {
    ...
    // Use raw track string for file system checks; normalization is applied later
    // upon retrieval
