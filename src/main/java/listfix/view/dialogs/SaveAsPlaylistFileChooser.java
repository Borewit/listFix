...
/**
 * Override the behaviour of the JFileChooser. It preserves the filename entered by the user when a
 * different file-extension is chosen.
 */
    ...
    this.addPropertyChangeListener(
        propertyChangeEvent -> {
          if (this.isVisible()
            ...
            SpecificPlaylistFileFilter fileFilter =
                (SpecificPlaylistFileFilter) propertyChangeEvent.getNewValue();
            if (currentFileName != null) {
              ...
              if (!fileFilter.getContentType().accept(userFile)) {
                String nameWithoutExtension =
                    FileUtils.getExtension(currentFileName)
                        .map(ext -> currentFileName.substring(0, currentFileName.lastIndexOf(".")))
                        .orElse(currentFileName);
                String newName =
                    nameWithoutExtension + fileFilter.getContentType().getExtensions()[0];
