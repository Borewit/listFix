...
public class ProgressAdapter<T> {
  ...
  private static final IProgressObserver<Object> dummyObserver =
      new IProgressObserver<>() {
        @Override
  ...
  public static <T> ProgressAdapter<T> make(IProgressObserver<T> observer) {
    return new ProgressAdapter<>(
        observer == null ? (IProgressObserver<T>) dummyObserver : observer);
  }
  ...
  public void setTotal(long total) {
    ...
    if (report) this.observer.reportProgress(percentComplete);
  }
