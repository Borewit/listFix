...
/**
 * doInBackgroiund() - perform work process() - made this abstract to force an override in any
 * derived classes (worker is useless without it)
 */
public abstract class DualProgressWorker<T, V>
    extends SwingWorker<T, DualProgressWorker.ProgressItem<V>> implements IDualProgressObserver<V> {
  @Override
  ...
  protected void getEffectiveItems(
      List<ProgressItem<V>> items, ProgressItem<V> titem, ProgressItem<V> oitem) {
    ...
    for (int ix = items.size() - 1; ix >= 0; ix--) {
      ...
      if (other.state == null && item.state != null) other.state = item.state;
      if (titem.percentComplete >= 0
          && oitem.percentComplete >= 0
          && titem.state != null
          && oitem.state != null) break;
    }
  }
}
