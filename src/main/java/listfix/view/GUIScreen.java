...
public final class GUIScreen extends JFrame implements IListFixGui {
  ...
  private void postInitComponents() {
    ...
    if (this._listFixController.getShowMediaDirWindow()) {
      JOptionPane.showMessageDialog(
          ...
          new JTransparentTextArea(
              "You need to add a media directory before you can find the new locations of your files.  See help for more information."),
          "Reminder",
          JOptionPane.INFORMATION_MESSAGE);
    } else {
      _lstMediaLibraryDirs.setListData(
          new Vector<>(_listFixController.getMediaLibrary().getMediaDirectories()));
    }
    ...
    _playlistDirectoryTree.setTransferHandler(
        new FileListTransferHandler() {
          @Override
  ...
  private void recheckStatusOfOpenPlaylists() {
    this._playlistTabbedPane.getPlaylistEditors().stream()
        ...
        .forEach(Playlist::updateModifiedStatus);
  }
    ...
    KeyEventPostProcessor pp = new KeyEventPostProcessor() {
      ...
      private boolean ctrlShiftTabWasPressed(KeyEvent e) {
        return (e.getKeyCode() == KeyEvent.VK_TAB)
            ...
            && _playlistTabbedPane.getDocumentCount() > 0
      ...
      private void handleMouseEvent(MouseEvent e) {
        ...
        if (e.isPopupTrigger()) {
          ...
          if (_playlistDirectoryTree.getSelectionCount() > 0) {
            ...
            _miRenameSelectedItem.setEnabled(
                singleFileSelected
                    && !allTopLevel
                    && Files.isRegularFile(
                        GUIScreen.this.getSelectedPlaylistTreeNodes().get(0).getUserObject()));
  ...
  private void initComponents() {
    ...
    _miRemovePlaylistDirectory.addActionListener(
        evt -> this.removePlaylistDirectory(this._playlistDirectoryTree.getSelectionPaths()));
    ...
    _miOpenSelectedPlaylistLocation.addActionListener(
        evt -> this.openPlaylistFoldersFromPlaylistTree());
    ...
    statusLabel.setText(
        "Untitled List     Number of entries in list: 0     Number of lost entries: 0     Number of URLs: 0     Number of open playlists: 0");
    ...
    _mediaLibraryPanel.setBorder(
        BorderFactory.createTitledBorder(
            null, "Media Directories", TitledBorder.LEFT, TitledBorder.TOP));
    ...
    _refreshMediaDirsButton.setToolTipText(
        "The contents of your media library are cached; refresh to pickup changes");
    ...
    _lstMediaLibraryDirs.setTransferHandler(
        new FileListTransferHandler() {
          @Override
          public boolean handleFileList(List<File> fileList) {
            return fileList.stream()
    ...
    _playlistDirectoryPanel.setBorder(
        BorderFactory.createTitledBorder(
            null, "Playlists Directories", TitledBorder.LEFT, TitledBorder.TOP));
    ...
    _playlistDirectoryTree.addKeyListener(
        new KeyAdapter() {
          @Override
    ...
    _btnSetPlaylistsDir.setToolTipText(
        "Adds another playlist directory (folder) to the configuration");
    ...
    _docTabPanel.setBorder(
        BorderFactory.createTitledBorder(
            null, "Playlists Editor", TitledBorder.LEFT, TitledBorder.TOP));
    ...
    _newPlaylistMenuItem.setAccelerator(
    ...
    _closeAllMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(
            KeyEvent.VK_W, InputEvent.SHIFT_DOWN_MASK | InputEvent.CTRL_DOWN_MASK));
    ...
    _saveAsMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(
    ...
    _saveAllMenuItem.setAccelerator(
        KeyStroke.getKeyStroke(
    ...
    _clearHistoryMenuItem.setAccelerator(
    ...
    _appOptionsMenuItem.setAccelerator(
    ...
    _miClosestMatchRepairOpenPlaylists.setAccelerator(
        KeyStroke.getKeyStroke(
            KeyEvent.VK_R,
    ...
    _updateCheckMenuItem.setAccelerator(
    ...
    _playlistTabbedPane.addDocumentChangeListener(
        new IDocumentChangeListener() {
          @Override
          ...
          public void documentClosed(JPlaylistComponent playlistComponent) {
            ...
            if (_playlistTabbedPane.getDocumentCount() == 0) {
              ((CardLayout) _playlistPanel.getLayout())
                  .show(_playlistPanel, "_gettingStartedPanel");
              currentTabChanged();
              updateMenuItemStatuses();
            }
          }
    ...
    for (TreePath selPath : this.getSelectedPlaylistTreePaths()) {
      ...
      if (Files.isDirectory(toDelete)) {
        try {
          ...
          JOptionPane.showMessageDialog(
              this, message, "Deleting Playlist Failed", JOptionPane.WARNING_MESSAGE);
        }
        ...
        if (Files.isWritable(toDelete)) {
          ...
          JOptionPane.showMessageDialog(
              this,
              String.format("Failed to delete playlist: %s", fileShortname),
  ...
  public List<TreePath> getSelectedPlaylistTreePaths() {
    int[] selRows = _playlistDirectoryTree.getSelectionRows();
    if (selRows != null) {
      return Arrays.stream(selRows)
          .mapToObj(i -> _playlistDirectoryTree.getPathForRow(i))
          .collect(Collectors.toList());
    }
    return Collections.emptyList();
  }
  ...
  public List<PlaylistTreeNode> getSelectedPlaylistTreeNodes() {
    return this.getSelectedPlaylistTreePaths().stream()
        .map(path -> (PlaylistTreeNode) path.getLastPathComponent())
        .collect(Collectors.toList());
  }
  ...
  private void renameTreeSelectedNode() {
    ...
    if (selectedTreeNodes.size() != 1) {
      _logger.debug(
          String.format(
              "Will only rename when exactly 1 file is selected, got %s.",
              selectedTreeNodes.size()));
      return;
    }
    ...
    for (PlaylistTreeNode treeNode : selectedTreeNodes) {
      ...
      if (reply != null && !reply.equals("")) {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              this,
              new JTransparentTextArea("Failed to rename file."),
              "File playlist failed",
  ...
  private void openPlaylistFoldersFromPlaylistTree() {
    this.getSelectedFilesFromTreePlaylists().forEach(this::openFileLocation);
  }
  ...
  public void openPathListDrop(List<Path> droppedFiles) {
    try {
      ...
      JOptionPane.showMessageDialog(
          this,
          new JTransparentTextArea(
              ExStack.textFormatErrorForUser(
                  "There was a problem opening the file you selected.", ioe.getCause())),
          "Open Playlist Error",
          JOptionPane.ERROR_MESSAGE);
    }
  }
  ...
  public void openPlaylist(final Path playlistPath) {
    ...
    this._playlistTabbedPane.getPlaylistEditors().stream()
        ...
        .findAny()
        .ifPresent(list -> _playlistTabbedPane.setActivePlaylist(list));

    try {
      if (!Files.exists(playlistPath)) {
        JOptionPane.showMessageDialog(
            this,
            new JTransparentTextArea("File Not Found."),
            "Open Playlist Error",
            JOptionPane.ERROR_MESSAGE);
        return;
      }
      ...
      JOptionPane.showMessageDialog(
          ...
          new JTransparentTextArea(
              ExStack.textFormatErrorForUser(
                  "There was a problem opening the file you selected.", ex.getCause())),
          "Open Playlist Error",
          JOptionPane.ERROR_MESSAGE);
      return;
    }
    ...
    if (tempComp == null) {
        ...
        protected void done() {
          ...
          try {
            ...
            JOptionPane.showMessageDialog(
                ...
                new JTransparentTextArea(
                    ExStack.textFormatErrorForUser(
                        "There was a problem opening the file you selected, are you sure it was a playlist?",
                        ex.getCause())),
                "Open Playlist Error",
                JOptionPane.ERROR_MESSAGE);
            return;
      ...
      ProgressDialog pd = new ProgressDialog(
          ...
          worker,
          "Loading '"
              + (filename.length() > 70 ? filename.substring(0, 70) : filename)
              + "'...",
          textOnly,
          true);
      pd.setVisible(true);
    ...
    if (list.isNew()) {
      ...
      try {
        ...
        JOptionPane.showMessageDialog(
            this,
            new JTransparentTextArea(
                "Sorry, there was an error saving your playlist.  Please try again, or file a bug report."));
      } finally {
        setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
      }
    }
    ...
    if (rc == JFileChooser.APPROVE_OPTION) {
      ...
      if (newPlaylistFile.exists()) {
        int result = JOptionPane.showConfirmDialog(
            this,
            new JTransparentTextArea(
                "You picked a file that already exists, should I really overwrite it?"),
            "File Exists Warning",
      ...
      if (!targetFileFilter.getPlaylistProvider().getId().equals(playlist.getType().name())) {
        _logger.info(
            String.format(
                "Conversion of playlist is requested from %s to %s",
                playlist.getType(), targetFormat));
      }
  ...
  @Override
  public void savePlaylist(Playlist playlist, File saveAsFile, PlaylistFormat format)
      throws InterruptedException, IOException, ExecutionException {
    this.savePlaylist(playlist, saveAsFile.toPath(), format);
  }

  public void savePlaylist(Playlist playlist, Path saveAsPath, PlaylistFormat format)
      throws InterruptedException, IOException, ExecutionException {
  ...
  private void onPlaylistModified(Playlist list) {
    if (list != null) {
      ...
      String txt = String.format(
          ...
          list.isModified() ? "*" : "",
          list.size(),
          list.getMissingCount(),
          list.getUrlCount(),
          _playlistTabbedPane.getDocumentCount());
    ...
    if (playlist.isModified()) {
      ...
      if (rc == 0) {
        ...
        try {
          ...
          JOptionPane.showMessageDialog(
              this, ex.getCause(), "Save Error", JOptionPane.ERROR_MESSAGE);
        }
        return false;
      } else if (rc == 1) {
  ...
  private void confirmCloseApp() {
    ...
    if (!modifiedPlaylists.isEmpty()) {
      ...
      int rc = JOptionPane.showOptionDialog(
          this,
          new JTransparentTextArea(
              "You have unsaved changes. Do you really want to discard these changes and exit?"),
          "Confirm Close",
          JOptionPane.YES_NO_OPTION,
          JOptionPane.QUESTION_MESSAGE,
          null,
    ...
    appState.setActivePlaylistIndex(
        openPlaylists.isEmpty() ? null : _playlistTabbedPane.getSelectedIndex());
  ...
  private void addMediaFolder(File mediaFolderToAdd) {
    ...
    if (ArrayFunctions.containsStringPrefixingAnotherString(
        _listFixController.getMediaLibrary().getMediaDirectories(),
        dir,
        !ListFixController.FILE_SYSTEM_IS_CASE_SENSITIVE)) {
      JOptionPane.showMessageDialog(
          this,
          new JTransparentTextArea(
              "The directory you attempted to add is a subdirectory of one already in your media library, no change was made."),
          "Notification",
          JOptionPane.INFORMATION_MESSAGE);
      return;
    } else {
  ...
  private void _removeMediaDirButtonActionPerformed() {
    ...
    try {
      ...
      JOptionPane.showMessageDialog(
          this,
          new JTransparentTextArea(
              "An error has occured, files in the media directory you removed may not have been completely removed from the library.  Please refresh the library."));
      _logger.warn(e);
    }
    updateMediaDirButtons();
  }
  ...
  private void refreshMediaDirs() {
    ...
    try {
      worker.get();
      _lstMediaLibraryDirs.setListData(
          _listFixController.getMediaLibrary().getMediaDirectories().toArray(new String[] {}));
    } catch (InterruptedException | CancellationException ex) {
  ...
  private void _newIconButtonActionPerformed() {
    try {
      ...
      JOptionPane.showMessageDialog(
          ...
          new JTransparentTextArea(
              ExStack.textFormatErrorForUser(
                  "Sorry, there was an error creating a new playlist.  Please try again, or file a bug report.",
                  ex.getCause())),
          "New Playlist Error",
          JOptionPane.ERROR_MESSAGE);
    }
  ...
  private void removePlaylistDirectory(Collection<Path> selectedPath) {
    selectedPath.forEach(
        playlistDirectory -> {
          _logger.info(
              String.format(
                  "Removing playlist directory from configuration: %s", playlistDirectory));
  ...
  public static void main(String[] args)
    ...
    SwingUtilities.invokeAndWait(
        () -> {
          ...
          Thread.currentThread()
              .setUncaughtExceptionHandler((thread, e) -> _logger.error("Uncaught Exception", e));
        });
    ...
    if (mainWindow.getLocation().equals(new Point(0, 0))) {
      ...
      mainWindow.setLocation(
          dm.getWidth() / 2 - (labelSize.width / 2), dm.getHeight() / 2 - (labelSize.height / 2));
    }
  ...
  public void addPlaylistFolder(File playlistFile) {
    if (playlistFile.exists()) {
      ...
      JOptionPane.showMessageDialog(
          this, new JTransparentTextArea("The directory you selected/entered does not exist."));
    }
    try {
      this._listFixController.getApplicationConfiguration().write();
    } catch (IOException e) {
  ...
  private void fireSaveAllDialog() {
    ...
    if (dialog.getResultCode() == SaveAllDialog.OK) {
      ...
      ProgressWorker<Void, String> worker = new ProgressWorker<>() {
        @Override
        protected Void doInBackground() throws Exception {
          // this.setMaximum(editors.size()); // Removed - ProgressDialog handles max
          // based on setProgress calls
          int count = 0;
          for (PlaylistEditCtrl editor : editors) {
            ...
            try {
              ...
              // Currently, this will use the default encoding behavior of the library for the
              // selected format.
              // Explicitly passing selectedEncoding is not supported by the current
              // savePlaylist method signature.
              ...
              // This requires the savePlaylist method to NOT modify the internal state if
              // path/format match.
              if (newPath.equals(originalPath) && selectedFormat == playlist.getType()) {
                ...
                // User might expect the tab to update, but that's complex (requires
                // reloading/renaming tab).
                // For now, just save the file.
              }

            } catch (Exception e) {
              String errorMsg = String.format("Failed to save playlist %s to %s", originalFileName,
                  newPath.getFileName());
