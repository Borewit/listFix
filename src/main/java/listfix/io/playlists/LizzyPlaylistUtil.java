...
public class LizzyPlaylistUtil {
            ...
            contentTypes -> {
              Arrays.stream(contentTypes)
                  .forEach(
                      contentType ->
                          extensions.addAll(
                              Arrays.stream(contentType.getExtensions())
  ...
  public static List<SpecificPlaylistFileFilter> getPlaylistExtensionFilters() {
    ...
    specificPlaylistFactory.getProviders().stream()
        .forEach(
            provider -> {
              Arrays.stream(provider.getContentTypes())
                  .forEach(
                      contentType -> {
                        SpecificPlaylistFileFilter filter =
                            new SpecificPlaylistFileFilter(provider, contentType);
                        filters.add(filter);
                      });
            });
    return filters;
  ...
  public static SpecificPlaylist toPlaylist(PlaylistFormat playlistFormat, Playlist playlist) {
    ...
    if (playlistProvider == null) throw new RuntimeException("Cannot not find provider ");
    try {
  ...
  /**
   * Read playlist from InputStream. Does not close the inputStream!
   *
   * @param playlistFormat Source playlist type, e.g. "mp3".
   * @param inputStream Stream to read from.
   * @return Lizzy Playlist.
   */
  public static SpecificPlaylist readPlaylistFromInputStream(
      PlaylistFormat playlistFormat, InputStream inputStream) {
    SpecificPlaylistProvider specificPlaylistProvider =
        specificPlaylistFactory.getProvider(playlistFormat);
    try {
  ...
  public static SpecificPlaylist writeNewPlaylist(
      Playlist playlist, Path path, PlaylistFormat playlistFormat, OpenOption... options)
      throws IOException {
    try (OutputStream outputStream = Files.newOutputStream(path, options)) {
      SpecificPlaylist specificPlaylist =
          specificPlaylistFactory.getProvider(playlistFormat).toSpecificPlaylist(playlist);
  ...
  public static boolean providerAccepts(SpecificPlaylistProvider provider, String filename) {
    ...
    return Arrays.stream(provider.getContentTypes())
        .anyMatch(types -> types.matchExtension(normalizedFilename));
  }

  public static String getPreferredExtensionFor(PlaylistFormat playlistFormat) {
    return specificPlaylistFactory.getProvider(playlistFormat)
        .getContentTypes()[0]
        .getExtensions()[0];
  }
