    ...
    if (Desktop.isDesktopSupported())
    {
      ...
      else if (desktop.isSupported(Desktop.Action.OPEN))
      {
        ...
        catch (IOException e)
        {
          ...
          JOptionPane.showMessageDialog(parent, new JTransparentTextArea(ExStack.textFormatErrorForUser(message, e.getCause())),
            "Failed to open file location", JOptionPane.ERROR_MESSAGE);
          return;
        }
      }
    }
    JOptionPane.showMessageDialog(parent, "Could not find supported desktop action to open file",
      "Failed to open file location", JOptionPane.ERROR_MESSAGE);
  }
}
