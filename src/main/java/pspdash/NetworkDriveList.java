...
// Copyright (C) 2002-2004 Software Process Dashboard Initiative
//
...
/**
 * On Windows systems, this class compiles a list of shared directories and drive letters that are
 * mapped to network drives, along with the UNC names they are mapped to. It can then be used to
 * translate filenames back and forth between drive letter syntax and UNC syntax.
 */
public class NetworkDriveList {
  ...
  /** Get a list of network drives. Uses a default maximum delay of 3 seconds. */
   ...
   *
   * @param maxDelay the maximum number of milliseconds to spend creating the drive list. If the
   *     operation takes longer than this, the resulting drive list will be empty, and
   *     wasSuccessful() will return false.
   */
  public NetworkDriveList(int maxDelay) {
    if (OperatingSystem.isWindows()) {
      Thread t = new Thread(this::getList);
      t.setDaemon(true);
      t.start();
      try {
        t.join(maxDelay);
  ...
  /** Find the drives mapped on the current system, and add them to the list. */
  private void listMappedDrives() throws Exception {
    synchronized (this) {
      subprocess = Runtime.getRuntime().exec("net use");
    }
    BufferedReader in =
        new BufferedReader(new InputStreamReader(subprocess.getInputStream(), UTF_8));
    String line;
    ...
    while ((line = in.readLine()) != null) {
      if (!sawHeader && line.startsWith("-----")) {
        ...
        if (" \t".indexOf(line.charAt(pos - 2)) == -1) {
          continue;
        }
  ...
  /** Find the local directories which are shared as networked drives, and add them to the list. */
    ...
    while ((line = in.readLine()) != null) {
      if (!sawHeader && line.startsWith("-----")) {
        ...
        if (shareName.endsWith("$")) {
          continue;
        } // don't include the "default" shares
   ...
   *
   * <p>On Windows systems, this will return false if the operation took too long and was abandoned.
   *
   * <p>On non-Windows systems, this will always return false.
   */
   ...
   *
   * <p>If wasSuccessful() returns false, this will generally return false.
   */
  ...
  /**
   * Get the UNC name associated with a particular drive letter. If isNetworkDrive() returns false
   * for this drive letter, returns null.
   */
   ...
   *
   * @return a filename beginning with a drive letter, or null if the filename could not be
   *     translated.
   */
  public String fromUNCName(String uncName) {
    ...
    if (uncName == null || !uncName.startsWith("\\\\")) {
      return null;
    }
  ...
  private String getDriveLetter(String filename) {
    ...
    if (filename.length() == 1 && "ABCDEFGHIJKLMNOPQRSTUVWXYZ".indexOf(filename.charAt(0)) != -1) {
      return filename;
    }
