 ...
 * </ul>
 * Note that this class works best if directory filenames end with a separator. If you omit the last separator, it is impossible to determine if the filename corresponds to a file or a directory. As a
 * result, we have chosen to say it corresponds to a file.
 * <p>
 ...
 * </pre> Both prefix styles are matched always, irrespective of the machine that you are currently running on.
 * <p>
...
public class FilenameUtils
{
	 ...
	 * segment and the one before to be removed. If the double dot has no parent path segment to work with, {@code null} is returned.
	 * <p>
	 ...
	 * ~/foo/../bar/        --&gt;   ~/bar/
	 * ~/../bar             --&gt;   null
	 * </pre> (Note the file separator returned will be correct for Windows/Unix)
	 *
	 ...
	 * ~/foo/../bar/        --&gt;   ~/bar/
	 * ~/../bar             --&gt;   null
	 * </pre> The output will be the same on both Unix and Windows including the separator character.
	 *
	 * @param filename the filename to normalize, null returns null
	 ...
	 *
	 * @param filename the filename to normalize, null returns null
	 * @param unixSeparator {@code true} if a unix separator should be used or {@code false} if a windows separtor should be used.
	 * @return the normalized filename, or null if invalid
	 * @since 2.0
	 */
	 ...
	 * <p>
	 * If
	 * <code>pathToAdd</code> is absolute (has an absolute prefix), then it will be normalized and returned. Otherwise, the paths will be joined, normalized and returned.
	 * <p>
	 * The output will be the same on both Unix and Windows except for the separator character.
	 * <pre>
	 * /foo/ + bar          --&gt;   /foo/bar
	 ...
	 * </pre> (*) Note that the Windows relative drive prefix is unreliable when used with this method. (!) Note that the first parameter must be a path. If it ends with a name, then the name will be
	 * built into the concatenated path. If this might be a problem, use {@link #getFullPath(String)} on the base path argument.
	 *
	 ...
	 *
	 * Note that a leading // (or \\) is used to indicate a UNC name on Windows. These must be followed by a server name, so double-slashes are not collapsed to a single slash at the start of the
	 * filename.
	 *
	 ...
	 * <p>
	 * This method obtains the extension as the textual part of the filename after the last dot. There must be no directory separator after the dot. The extension check is case-sensitive on all
	 * platforms.
	 *
	 ...
	 * <p>
	 * The wildcard matcher uses the characters '?' and '*' to represent a single or multiple (zero or more) wildcard characters. This is the same as often found on Dos/Unix command lines. The check
	 ...
	 * </pre> N.B. the sequence "*?" does not work properly at present in match strings.
	 *
	 ...
	 * <p>
	 * The wildcard matcher uses the characters '?' and '*' to represent a single or multiple (zero or more) wildcard characters. N.B. the sequence "*?" does not work properly at present in match
	 * strings.
	 *
	 * @param filename the filename to match on
	 * @param wildcardMatcher the wildcard string to match against
